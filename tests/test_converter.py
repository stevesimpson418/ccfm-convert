...
class TestFencedExpandDirective:
    """Test :::expand fenced directive syntax."""

    def test_basic_expand(self):
        """Test basic :::expand directive produces an expand node."""
        markdown = ":::expand Section Title\nContent here\n:::"
        result = convert(markdown)

        node = result["content"][0]
        assert node["type"] == "expand"
        assert node["attrs"]["title"] == "Section Title"
    ...
    def test_expand_with_code_block(self):
        ...
        markdown = ":::expand Python example\n```python\nprint('hello')\n```\n:::"
        result = convert(markdown)

        node = result["content"][0]
        assert node["type"] == "expand"
        assert node["attrs"]["title"] == "Python example"
        # Should contain a code block
        code = [n for n in node["content"] if n["type"] == "codeBlock"]
        assert len(code) == 1
        assert code[0]["content"][0]["text"] == "print('hello')"

    def test_expand_with_blank_lines(self):
        """Test expand with blank lines in content."""
    ...
    def test_expand_between_other_blocks(self):
        """Test expand surrounded by other content."""
        markdown = "Before\n\n:::expand Title\nInside\n:::\n\nAfter"
        result = convert(markdown)
    ...
    def test_expand_without_closing_fence(self):
        """Test expand with missing closing ::: consumes to end of document."""
        markdown = ":::expand Unclosed\nContent"
        result = convert(markdown)

        node = result["content"][0]
        assert node["type"] == "expand"
        assert node["attrs"]["title"] == "Unclosed"

    def test_expand_breaks_paragraph(self):
        """Test that :::expand breaks a paragraph."""
    ...
    def test_expand_empty_body(self):
        """Test expand with no content."""
        markdown = ":::expand Empty\n:::"
        result = convert(markdown)
