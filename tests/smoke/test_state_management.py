...
class TestPlanMode:
    ...
    def test_plan_before_apply_shows_adds(self, ccfm_run, confluence_live):
        ...
        result = ccfm_run(
            "plan", "--force", "--plan-exit-code", "--config", str(STATE_CONFIG), check=False
        )
    ...
    def test_plan_after_apply_shows_no_changes(self, ccfm_run, confluence_live):
        ...
        result = ccfm_run("apply", "--auto-approve", "--config", str(STATE_CONFIG))
        assert result.returncode == 0, f"Initial apply failed:\n{result.stderr}"

        result = ccfm_run("plan", "--config", str(STATE_CONFIG), check=False)
